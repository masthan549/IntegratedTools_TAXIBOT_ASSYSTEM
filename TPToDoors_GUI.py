...
class GUI_COntroller:
    ...
    def openfile():
        ...
        if len(filepath):
            ...
            EntryObj2.place(x=330,y=200)			
			
			
            button5 = Button(TkObject_ref,text='Run To Dooors Format',font=10,bd=5,command=TestScript.RunTest)
...
class TestScript:
    def RunTest():
        ...
        if len(systemExeTime):
            TPToDoors_Script.startFun(filepath,filepath_temp,TkObject_ref,systemExeTime)

def TPTemplateToDoors():	
       ...
       root.title('Welcome to TP Template to Doors conversion tool')
